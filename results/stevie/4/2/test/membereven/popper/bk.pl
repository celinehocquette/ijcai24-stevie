...
ho_12(A,P)  :- head(A,B),tail(A,C),ho_12(C,P),head(C,D),call(P,B,D).

ho_288(A,P)  :- head(A,B),call(P,B).

ho_288(A,P)  :- tail(A,B),ho_288(B,P).

ho_155(A,B,P,Q)  :- empty(A),zero_int(B).

ho_155(A,B,P,Q)  :- head(A,C),tail(A,D),call(P,C),ho_155(D,B,P,Q).

ho_155(A,B,P,Q)  :- head(A,C),tail(A,D),call(Q,C),ho_155(D,E,P,Q),my_increment(E,B).
...
ho_499(A,B,C,P)  :- decrement(A,D),ho_499(D,B,E,P),call(P,E,C).

ho_317(A,P)  :- empty(A).

ho_317(A,P)  :- head(A,B),tail(A,C),call(P,B),ho_317(C,P).
...
ho_273(A,B,P,Q)  :- cons3(A,C,D),call(P,C),ho_273(D,B,P,Q).

ho_273(A,B,P,Q)  :- cons3(A,C,D),call(Q,C),ho_273(D,E,P,Q),cons(C,E,B).
