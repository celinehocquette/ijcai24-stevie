...
ho_175(A,B,P,Q)  :- cons3(A,C,D),call(P,C),ho_175(D,B,P,Q).

ho_175(A,B,P,Q)  :- cons3(A,C,D),call(Q,C),ho_175(D,E,P,Q),cons(C,E,B).

ho_23(A,P)  :- empty(A).

ho_23(A,P)  :- head(A,B),tail(A,C),call(P,B),ho_23(C,P).

ho_62(A,P)  :- tail(A,B),empty(B).

ho_62(A,P)  :- head(A,B),tail(A,C),ho_62(C,P),head(C,D),call(P,B,D).

ho_1(A,B,P)  :- call(P,A,C),call(P,C,D),call(P,D,E),call(P,E,F),call(P,F,B).

ho_43(A,B,P,Q)  :- empty(A),call(P,B).

ho_43(A,B,P,Q)  :- head(A,C),tail(A,D),ho_43(D,E,P,Q),call(Q,C,E,B).
...
ho_288(A,B,P,Q)  :- head(A,C),tail(A,D),call(P,C),ho_288(D,B,P,Q).

ho_288(A,B,P,Q)  :- head(A,C),tail(A,D),call(Q,C),ho_288(D,E,P,Q),my_increment(E,B).

ho_2(A,B,C,P)  :- zero_in(A),eq_list(B,C).

ho_2(A,B,C,P)  :- decrement(A,D),ho_2(D,B,E,P),call(P,E,C).

ho_142(A,B,P)  :- empty(A),empty(B).

ho_142(A,B,P)  :- head(A,C),tail(A,D),head(B,E),tail(B,F),call(P,C,E),ho_142(D,F,P).
