...
ho_144(A,B,P)  :- head(A,C),tail(A,D),head(B,E),tail(B,F),call(P,C,E),ho_144(D,F,P).
...
ho_279(A,B,P,Q)  :- head(A,C),tail(A,D),call(P,C),ho_279(D,B,P,Q).

ho_279(A,B,P,Q)  :- head(A,C),tail(A,D),call(Q,C),ho_279(D,E,P,Q),my_increment(E,B).

ho_24(A,P)  :- empty(A).

ho_24(A,P)  :- head(A,B),tail(A,C),call(P,B),ho_24(C,P).

ho_51(A,B,P,Q)  :- empty(A),call(P,B).

ho_51(A,B,P,Q)  :- head(A,C),tail(A,D),ho_51(D,E,P,Q),call(Q,C,E,B).
