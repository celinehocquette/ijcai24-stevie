...
ho_183(A,B,P,Q)  :- cons3(A,C,D),call(P,C),ho_183(D,B,P,Q).

ho_183(A,B,P,Q)  :- cons3(A,C,D),ho_183(D,E,P,Q),call(Q,C),cons(C,E,B).

ho_25(A,P)  :- empty(A).

ho_25(A,P)  :- head(A,B),tail(A,C),call(P,B),ho_25(C,P).

ho_3(A,P)  :- head(A,B),call(P,B).

ho_3(A,P)  :- tail(A,B),ho_3(B,P).

ho_113(A,B,P,Q)  :- empty(A),zero_int(B).

ho_113(A,B,P,Q)  :- head(A,C),tail(A,D),call(P,C),ho_113(D,B,P,Q).

ho_113(A,B,P,Q)  :- head(A,C),tail(A,D),call(Q,C),ho_113(D,E,P,Q),my_increment(E,B).

ho_70(A,B,P,Q)  :- call(P,A,B),head(B,C),call(Q,C).

ho_70(A,B,P,Q)  :- call(P,A,C),ho_70(C,B,P,Q).

ho_164(A,B,P)  :- empty(A),empty(B).

ho_164(A,B,P)  :- head(A,C),tail(A,D),head(B,E),tail(B,F),call(P,C,E),ho_164(D,F,P).
