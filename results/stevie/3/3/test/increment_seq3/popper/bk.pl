...
ho_7(A,P)  :- head(A,B),call(P,B).

ho_7(A,P)  :- tail(A,B),ho_7(B,P).

ho_29(A,P)  :- empty(A).

ho_29(A,P)  :- head(A,B),tail(A,C),call(P,B),ho_29(C,P).

ho_74(A,B,P,Q)  :- call(P,A,B),head(B,C),call(Q,C).

ho_74(A,B,P,Q)  :- call(P,A,C),ho_74(C,B,P,Q).

ho_118(A,B,P,Q)  :- empty(A),zero_int(B).

ho_118(A,B,P,Q)  :- head(A,C),tail(A,D),call(P,C),ho_118(D,B,P,Q).

ho_118(A,B,P,Q)  :- head(A,C),tail(A,D),call(Q,C),ho_118(D,E,P,Q),my_increment(E,B).

ho_185(A,B,P,Q)  :- empty(A),empty(B).

ho_185(A,B,P,Q)  :- cons3(A,C,D),call(P,C),ho_185(D,B,P,Q).

ho_185(A,B,P,Q)  :- cons3(A,C,D),ho_185(D,E,P,Q),call(Q,C),cons(C,E,B).
...
ho_167(A,B,P)  :- head(A,C),tail(A,D),head(B,E),tail(B,F),call(P,C,E),ho_167(D,F,P).

ho_1(A,B,P)  :- call(P,A,C),call(P,C,D),call(P,D,E),call(P,E,F),call(P,F,B).

ho_45(A,B,C,P)  :- zero_in(A),eq_list(B,C).

ho_45(A,B,C,P)  :- decrement(A,D),ho_45(D,B,E,P),call(P,E,C).
