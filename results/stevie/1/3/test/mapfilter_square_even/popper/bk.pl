...
ho_23(A,B,C,P)  :- decrement(A,D),call(P,B,E),ho_23(D,E,C,P).

ho_511(A,B,P,Q)  :- empty(A),call(P,B).

ho_511(A,B,P,Q)  :- head(A,C),tail(A,D),ho_511(D,E,P,Q),call(Q,C,E,B).

ho_7(A,B,P,Q)  :- call(P,A,B),head(B,C),call(Q,C).

ho_7(A,B,P,Q)  :- call(P,A,C),ho_7(C,B,P,Q).

ho_322(A,B,P,Q)  :- empty(A),zero_int(B).

ho_322(A,B,P,Q)  :- head(A,C),tail(A,D),call(P,C),ho_322(D,B,P,Q).

ho_322(A,B,P,Q)  :- head(A,C),tail(A,D),call(Q,C),ho_322(D,E,P,Q),my_increment(E,B).

ho_100(A,B,P,Q)  :- empty(A),empty(B).

ho_100(A,B,P,Q)  :- cons3(A,C,D),call(P,C),ho_100(D,B,P,Q).

ho_100(A,B,P,Q)  :- cons3(A,C,D),call(Q,C),ho_100(D,E,P,Q),cons(C,E,B).
...
ho_155(A,P)  :- head(A,B),tail(A,C),call(P,B),ho_155(C,P).

ho_16(A,P)  :- head(A,B),call(P,B).

ho_16(A,P)  :- tail(A,B),ho_16(B,P).

ho_62(A,B,P)  :- empty(A),empty(B).

ho_62(A,B,P)  :- head(A,C),tail(A,D),head(B,E),tail(B,F),call(P,C,E),ho_62(D,F,P).

ho_45(A,B,P,Q)  :- empty(A),call(P,B).

ho_45(A,B,P,Q)  :- head(A,C),tail(A,D),ho_45(D,E,P,Q),call(Q,E,C,B).
