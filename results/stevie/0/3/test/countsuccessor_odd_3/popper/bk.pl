...
ho_491(A,B,P,Q)  :- cons3(A,C,D),call(P,C),ho_491(D,B,P,Q).

ho_491(A,B,P,Q)  :- cons3(A,C,D),call(Q,C),ho_491(D,E,P,Q),cons(C,E,B).

ho_148(A,B,P,Q)  :- empty(A),zero_int(B).

ho_148(A,B,P,Q)  :- head(A,C),tail(A,D),call(P,C),ho_148(D,B,P,Q).

ho_148(A,B,P,Q)  :- head(A,C),tail(A,D),call(Q,C),ho_148(D,E,P,Q),my_increment(E,B).

ho_308(A,P)  :- tail(A,B),empty(B).

ho_308(A,P)  :- head(A,B),tail(A,C),ho_308(C,P),head(C,D),call(P,B,D).

ho_105(A,B,P,Q)  :- empty(A),call(P,B).

ho_105(A,B,P,Q)  :- head(A,C),tail(A,D),ho_105(D,E,P,Q),call(Q,C,E,B).
...
ho_252(A,B,P,Q)  :- call(P,A,B),head(B,C),call(Q,C).

ho_252(A,B,P,Q)  :- call(P,A,C),ho_252(C,B,P,Q).
