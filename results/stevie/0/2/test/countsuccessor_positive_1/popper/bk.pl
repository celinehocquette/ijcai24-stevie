...
ho_41(A,P)  :- head(A,B),tail(A,C),call(P,B),ho_41(C,P).

ho_182(A,B,P)  :- empty(A),empty(B).

ho_182(A,B,P)  :- head(A,C),tail(A,D),head(B,E),tail(B,F),call(P,C,E),ho_182(D,F,P).

ho_15(A,B,C,P)  :- zero_in(A),eq_list(B,C).

ho_15(A,B,C,P)  :- decrement(A,D),call(P,B,E),ho_15(D,E,C,P).

ho_487(A,B,P,Q)  :- empty(A),empty(B).

ho_487(A,B,P,Q)  :- cons3(A,C,D),call(P,C),ho_487(D,B,P,Q).

ho_487(A,B,P,Q)  :- cons3(A,C,D),call(Q,C),ho_487(D,E,P,Q),cons(C,E,B).

ho_134(A,B,P,Q)  :- empty(A),zero_int(B).

ho_134(A,B,P,Q)  :- head(A,C),tail(A,D),call(P,C),ho_134(D,B,P,Q).

ho_134(A,B,P,Q)  :- head(A,C),tail(A,D),call(Q,C),ho_134(D,E,P,Q),my_increment(E,B).

ho_103(A,B,P,Q)  :- empty(A),call(P,B).

ho_103(A,B,P,Q)  :- head(A,C),tail(A,D),ho_103(D,E,P,Q),call(Q,C,E,B).
