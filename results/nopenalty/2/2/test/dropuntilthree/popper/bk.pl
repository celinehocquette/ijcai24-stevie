...
ho_33(A,B,C,P)  :- decrement(A,D),ho_33(D,B,E,P),call(P,E,C).

ho_178(A,B,P,Q,R)  :- empty(A),zero_int(B).

ho_178(A,B,P,Q,R)  :- call(P,A,C),tail(A,D),call(Q,C),ho_178(D,B,P,Q,R).

ho_178(A,B,P,Q,R)  :- call(P,A,C),tail(A,D),call(R,C),ho_178(D,E,P,Q,R),my_increment(E,B).

ho_366(A,B,P,Q,R)  :- empty(A),call(P,B).

ho_366(A,B,P,Q,R)  :- call(Q,A,C),tail(A,D),ho_366(D,E,P,Q,R),call(R,C,E,B).

ho_452(A,P,Q,R)  :- call(P,A,B),call(Q,B).

ho_452(A,P,Q,R)  :- head(A,B),call(P,A,C),ho_452(C,P,Q,R),head(C,D),call(R,B,D).

ho_316(A,B,P)  :- empty(A),empty(B).

ho_316(A,B,P)  :- head(A,C),tail(A,D),head(B,E),tail(B,F),call(P,C,E),ho_316(D,F,P).
...
ho_290(A,B,P,Q,R)  :- cons3(A,C,D),call(P,C),ho_290(D,B,P,Q,R).

ho_290(A,B,P,Q,R)  :- cons3(A,C,D),ho_290(D,E,P,Q,R),call(Q,C),call(R,C,E,B).
