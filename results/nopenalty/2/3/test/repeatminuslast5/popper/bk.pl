...
ho_14(A,B,P,Q)  :- call(P,A,B),head(B,C),call(Q,C).

ho_14(A,B,P,Q)  :- call(P,A,C),ho_14(C,B,P,Q).

ho_179(A,B,P,Q,R)  :- empty(A),zero_int(B).

ho_179(A,B,P,Q,R)  :- head(A,C),tail(A,D),call(P,C),ho_179(D,B,P,Q,R).

ho_179(A,B,P,Q,R)  :- head(A,C),tail(A,D),call(Q,C),ho_179(D,E,P,Q,R),call(R,E,B).

ho_361(A,B,P,Q)  :- empty(A),call(P,B).

ho_361(A,B,P,Q)  :- head(A,C),tail(A,D),ho_361(D,E,P,Q),call(Q,C,E,B).

ho_290(A,B,P,Q,R)  :- call(P,A),call(P,B).

ho_290(A,B,P,Q,R)  :- cons3(A,C,D),call(Q,C),ho_290(D,B,P,Q,R).

ho_290(A,B,P,Q,R)  :- cons3(A,C,D),ho_290(D,E,P,Q,R),call(R,C),cons(C,E,B).

ho_1(A,B,P)  :- call(P,A,C),call(P,C,D),call(P,D,E),call(P,E,F),call(P,F,B).

ho_38(A,B,C,P,Q)  :- zero_in(A),call(P,B,C).

ho_38(A,B,C,P,Q)  :- decrement(A,D),ho_38(D,B,E,P,Q),call(Q,E,C).

ho_447(A,P,Q)  :- call(P,A,B),empty(B).

ho_447(A,P,Q)  :- head(A,B),call(P,A,C),ho_447(C,P,Q),head(C,D),call(Q,B,D).

ho_319(A,B,P)  :- empty(A),empty(B).

ho_319(A,B,P)  :- head(A,C),tail(A,D),head(B,E),tail(B,F),call(P,C,E),ho_319(D,F,P).

ho_55(A,P,Q,R)  :- empty(A).

ho_55(A,P,Q,R)  :- call(P,A,B),call(Q,A,C),call(R,B),ho_55(C,P,Q,R).
