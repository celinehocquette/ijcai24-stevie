...
ho_53(A,B,P,Q,R)  :- call(P,A,C),call(Q,A,D),call(P,B,E),call(Q,B,F),call(R,C,E),ho_53(D,F,P,Q,R).

ho_59(A,B,P)  :- empty(A),zero(B).

ho_59(A,B,P)  :- head(A,C),tail(A,D),ho_59(D,E,P),call(P,C,E,B).

ho_1(A,B,P)  :- call(P,A,C),call(P,C,D),call(P,D,E),call(P,E,F),call(P,F,B).

ho_431(A,P)  :- tail(A,B),empty(B).

ho_431(A,P)  :- head(A,B),tail(A,C),ho_431(C,P),head(C,D),call(P,B,D).

ho_121(A,B,P,Q,R)  :- call(P,A,B),call(Q,B,C),call(R,C).

ho_121(A,B,P,Q,R)  :- call(P,A,C),ho_121(C,B,P,Q,R).

ho_171(A,P,Q)  :- call(P,A,B),call(Q,B).

ho_171(A,P,Q)  :- tail(A,B),ho_171(B,P,Q).

ho_181(A,B,C,P,Q)  :- zero_in(A),call(P,B,C).

ho_181(A,B,C,P,Q)  :- decrement(A,D),ho_181(D,B,E,P,Q),call(Q,E,C).

ho_160(A,B,P,Q,R)  :- empty(A),empty(B).

ho_160(A,B,P,Q,R)  :- call(P,A,C,D),call(Q,C),ho_160(D,B,P,Q,R).

ho_160(A,B,P,Q,R)  :- call(P,A,C,D),call(R,C),ho_160(D,E,P,Q,R),cons(C,E,B).

ho_242(A,B,P,Q,R)  :- empty(A),call(P,B).

ho_242(A,B,P,Q,R)  :- head(A,C),tail(A,D),call(Q,C),ho_242(D,B,P,Q,R).

ho_242(A,B,P,Q,R)  :- head(A,C),tail(A,D),call(R,C),ho_242(D,E,P,Q,R),my_increment(E,B).

ho_5(A,P)  :- empty(A).

ho_5(A,P)  :- head(A,B),tail(A,C),call(P,B),ho_5(C,P).
