...
ho_252(A,B,P,Q,R)  :- empty(A),call(P,B).

ho_252(A,B,P,Q,R)  :- head(A,C),tail(A,D),call(Q,C),ho_252(D,B,P,Q,R).

ho_252(A,B,P,Q,R)  :- head(A,C),tail(A,D),call(R,C),ho_252(D,E,P,Q,R),my_increment(E,B).

ho_15(A,P,Q,R)  :- call(P,A).

ho_15(A,P,Q,R)  :- head(A,B),call(Q,A,C),call(R,B),ho_15(C,P,Q,R).

ho_188(A,B,C,P,Q,R)  :- call(P,A),eq_list(B,C).

ho_188(A,B,C,P,Q,R)  :- call(Q,A,D),ho_188(D,B,E,P,Q,R),call(R,E,C).
...
ho_120(A,B,P,Q)  :- call(P,A,B),head(B,C),call(Q,C).

ho_120(A,B,P,Q)  :- call(P,A,C),ho_120(C,B,P,Q).

ho_54(A,B,P,Q,R)  :- call(P,A),call(P,B).

ho_54(A,B,P,Q,R)  :- head(A,C),call(Q,A,D),head(B,E),call(Q,B,F),call(R,C,E),ho_54(D,F,P,Q,R).
