...
ho_177(A,B,C,P,Q)  :- zero_in(A),call(P,B,C).

ho_177(A,B,C,P,Q)  :- decrement(A,D),call(Q,B,E),ho_177(D,E,C,P,Q).

ho_1(A,B,P)  :- call(P,A,C),call(P,C,D),call(P,D,E),call(P,E,F),call(P,F,B).

ho_358(A,B,P,Q,R)  :- empty(A),empty(B).

ho_358(A,B,P,Q,R)  :- call(P,A,C,D),call(Q,C),ho_358(D,B,P,Q,R).

ho_358(A,B,P,Q,R)  :- call(P,A,C,D),ho_358(D,E,P,Q,R),call(R,C),cons(C,E,B).

ho_161(A,P,Q)  :- call(P,A,B),call(Q,B).

ho_161(A,P,Q)  :- tail(A,B),ho_161(B,P,Q).

ho_24(A,B,P,Q,R)  :- empty(A),call(P,B).

ho_24(A,B,P,Q,R)  :- head(A,C),call(Q,A,D),ho_24(D,E,P,Q,R),call(R,C,E,B).

ho_142(A,B,P,Q,R)  :- empty(A),empty(B).

ho_142(A,B,P,Q,R)  :- call(P,A,C),call(Q,A,D),call(P,B,E),call(Q,B,F),call(R,C,E),ho_142(D,F,P,Q,R).
...
ho_115(A,B,P,Q,R)  :- empty(A),call(P,B).

ho_115(A,B,P,Q,R)  :- head(A,C),tail(A,D),call(Q,C),ho_115(D,B,P,Q,R).

ho_115(A,B,P,Q,R)  :- head(A,C),tail(A,D),call(R,C),ho_115(D,E,P,Q,R),my_increment(E,B).

ho_232(A,P,Q,R)  :- empty(A).

ho_232(A,P,Q,R)  :- call(P,A,B),call(Q,A,C),call(R,B),ho_232(C,P,Q,R).
