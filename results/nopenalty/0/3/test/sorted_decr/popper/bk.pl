...
ho_438(A,P,Q)  :- call(P,A,B),empty(B).

ho_438(A,P,Q)  :- head(A,B),call(P,A,C),ho_438(C,P,Q),head(C,D),call(Q,B,D).

ho_142(A,B,P,Q,R)  :- call(P,A),call(P,B).

ho_142(A,B,P,Q,R)  :- call(Q,A,C),tail(A,D),call(Q,B,E),tail(B,F),call(R,C,E),ho_142(D,F,P,Q,R).

ho_355(A,B,P,Q,R)  :- call(P,A),call(P,B).

ho_355(A,B,P,Q,R)  :- cons3(A,C,D),call(Q,C),ho_355(D,B,P,Q,R).

ho_355(A,B,P,Q,R)  :- cons3(A,C,D),ho_355(D,E,P,Q,R),call(R,C),cons(C,E,B).

ho_220(A,P)  :- empty(A).

ho_220(A,P)  :- head(A,B),tail(A,C),call(P,B),ho_220(C,P).

ho_251(A,B,C,P,Q,R)  :- zero_in(A),call(P,B,C).

ho_251(A,B,C,P,Q,R)  :- call(Q,A,D),ho_251(D,B,E,P,Q,R),call(R,E,C).

ho_113(A,B,P,Q,R)  :- empty(A),zero_int(B).

ho_113(A,B,P,Q,R)  :- head(A,C),tail(A,D),call(P,C),ho_113(D,B,P,Q,R).

ho_113(A,B,P,Q,R)  :- head(A,C),tail(A,D),call(Q,C),ho_113(D,E,P,Q,R),call(R,E,B).

ho_18(A,B,P,Q,R)  :- call(P,A),call(Q,B).

ho_18(A,B,P,Q,R)  :- head(A,C),tail(A,D),ho_18(D,E,P,Q,R),call(R,C,E,B).

ho_163(A,P,Q,R)  :- call(P,A,B),call(Q,B).

ho_163(A,P,Q,R)  :- call(R,A,B),ho_163(B,P,Q,R).

ho_171(A,B,C,P)  :- zero_in(A),eq_list(B,C).

ho_171(A,B,C,P)  :- decrement(A,D),call(P,B,E),ho_171(D,E,C,P).

ho_1(A,B,P)  :- call(P,A,C),call(P,C,D),call(P,D,E),call(P,E,F),call(P,F,B).

ho_239(A,B,P,Q,R)  :- call(P,A,B),call(Q,B,C),call(R,C).

ho_239(A,B,P,Q,R)  :- call(P,A,C),ho_239(C,B,P,Q,R).
