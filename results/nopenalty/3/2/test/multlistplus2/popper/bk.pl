...
ho_490(A,B,P,Q)  :- cons3(A,C,D),call(P,C),ho_490(D,B,P,Q).

ho_490(A,B,P,Q)  :- cons3(A,C,D),call(Q,C),ho_490(D,E,P,Q),cons(C,E,B).
...
ho_152(A,B,P,Q,R)  :- head(A,C),call(P,A,D),call(Q,C),ho_152(D,B,P,Q,R).

ho_152(A,B,P,Q,R)  :- head(A,C),call(P,A,D),call(R,C),ho_152(D,E,P,Q,R),my_increment(E,B).

ho_1(A,B,P)  :- call(P,A,C),call(P,C,D),call(P,D,E),call(P,E,F),call(P,F,B).

ho_26(A,P,Q,R)  :- empty(A).

ho_26(A,P,Q,R)  :- call(P,A,B),call(Q,A,C),call(R,B),ho_26(C,P,Q,R).

ho_305(A,B,P)  :- empty(A),one(B).

ho_305(A,B,P)  :- head(A,C),tail(A,D),ho_305(D,E,P),call(P,E,C,B).

ho_8(A,B,P,Q,R)  :- call(P,A,B),call(Q,B,C),call(R,C).

ho_8(A,B,P,Q,R)  :- call(P,A,C),ho_8(C,B,P,Q,R).

ho_57(A,P,Q,R)  :- call(P,A,B),call(Q,B).

ho_57(A,P,Q,R)  :- head(A,B),call(P,A,C),ho_57(C,P,Q,R),head(C,D),call(R,B,D).

ho_15(A,P,Q,R)  :- call(P,A,B),call(Q,B).

ho_15(A,P,Q,R)  :- call(R,A,B),ho_15(B,P,Q,R).
