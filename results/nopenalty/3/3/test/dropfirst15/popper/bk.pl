...
ho_195(A,B,C,P,Q)  :- zero_in(A),call(P,B,C).

ho_195(A,B,C,P,Q)  :- decrement(A,D),ho_195(D,B,E,P,Q),call(Q,E,C).

ho_44(A,P)  :- tail(A,B),empty(B).

ho_44(A,P)  :- head(A,B),tail(A,C),ho_44(C,P),head(C,D),call(P,B,D).

ho_104(A,B,P,Q)  :- empty(A),zero_int(B).

ho_104(A,B,P,Q)  :- head(A,C),tail(A,D),call(P,C),ho_104(D,B,P,Q).

ho_104(A,B,P,Q)  :- head(A,C),tail(A,D),call(Q,C),ho_104(D,E,P,Q),my_increment(E,B).

ho_312(A,B,P,Q)  :- empty(A),call(P,B).

ho_312(A,B,P,Q)  :- head(A,C),tail(A,D),ho_312(D,E,P,Q),call(Q,E,C,B).
...
ho_187(A,B,P,Q,R)  :- head(A,C),call(Q,A,D),head(B,E),call(Q,B,F),call(R,C,E),ho_187(D,F,P,Q,R).

ho_14(A,P,Q)  :- head(A,B),call(P,B).

ho_14(A,P,Q)  :- call(Q,A,B),ho_14(B,P,Q).

ho_25(A,P,Q)  :- call(P,A).

ho_25(A,P,Q)  :- head(A,B),tail(A,C),call(Q,B),ho_25(C,P,Q).
