...
ho_29(A,P,Q,R)  :- call(P,A).

ho_29(A,P,Q,R)  :- head(A,B),call(Q,A,C),call(R,B),ho_29(C,P,Q,R).

ho_324(A,B,P,Q,R)  :- call(P,A),one(B).

ho_324(A,B,P,Q,R)  :- head(A,C),call(Q,A,D),ho_324(D,E,P,Q,R),call(R,E,C,B).

ho_186(A,B,P,Q,R)  :- call(P,A),call(P,B).

ho_186(A,B,P,Q,R)  :- head(A,C),call(Q,A,D),head(B,E),call(Q,B,F),call(R,C,E),ho_186(D,F,P,Q,R).

ho_1(A,B,P)  :- call(P,A,C),call(P,C,D),call(P,D,E),call(P,E,F),call(P,F,B).

ho_9(A,P)  :- head(A,B),call(P,B).

ho_9(A,P)  :- tail(A,B),ho_9(B,P).

ho_147(A,B,P,Q,R)  :- empty(A),zero_int(B).

ho_147(A,B,P,Q,R)  :- head(A,C),tail(A,D),call(P,C),ho_147(D,B,P,Q,R).

ho_147(A,B,P,Q,R)  :- head(A,C),tail(A,D),call(Q,C),ho_147(D,E,P,Q,R),call(R,E,B).

ho_8(A,B,P,Q,R)  :- call(P,A,B),call(Q,B,C),call(R,C).

ho_8(A,B,P,Q,R)  :- call(P,A,C),ho_8(C,B,P,Q,R).

ho_55(A,P,Q,R)  :- call(P,A,B),empty(B).

ho_55(A,P,Q,R)  :- call(Q,A,B),call(P,A,C),ho_55(C,P,Q,R),call(Q,C,D),call(R,B,D).
