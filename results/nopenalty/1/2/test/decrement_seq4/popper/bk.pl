...
ho_34(A,P)  :- head(A,B),tail(A,C),call(P,B),ho_34(C,P).

ho_292(A,B,P,Q,R)  :- empty(A),zero_int(B).

ho_292(A,B,P,Q,R)  :- head(A,C),tail(A,D),call(P,C),ho_292(D,B,P,Q,R).

ho_292(A,B,P,Q,R)  :- head(A,C),tail(A,D),call(Q,C),ho_292(D,E,P,Q,R),call(R,E,B).

ho_1(A,B,P)  :- call(P,A,C),call(P,C,D),call(P,D,E),call(P,E,F),call(P,F,B).

ho_153(A,B,C,P,Q,R)  :- call(P,A),eq_list(B,C).

ho_153(A,B,C,P,Q,R)  :- call(Q,A,D),ho_153(D,B,E,P,Q,R),call(R,E,C).
