...
ho_139(A,P,Q)  :- call(P,A,B),call(Q,B).

ho_139(A,P,Q)  :- tail(A,B),ho_139(B,P,Q).

ho_73(A,B,C,P,Q)  :- zero_in(A),call(P,B,C).

ho_73(A,B,C,P,Q)  :- decrement(A,D),call(Q,B,E),ho_73(D,E,C,P,Q).

ho_303(A,B,P,Q,R)  :- empty(A),zero_int(B).

ho_303(A,B,P,Q,R)  :- call(P,A,C),tail(A,D),call(Q,C),ho_303(D,B,P,Q,R).

ho_303(A,B,P,Q,R)  :- call(P,A,C),tail(A,D),call(R,C),ho_303(D,E,P,Q,R),my_increment(E,B).

ho_22(A,B,P,Q,R)  :- call(P,A,B),call(Q,B,C),call(R,C).

ho_22(A,B,P,Q,R)  :- call(P,A,C),ho_22(C,B,P,Q,R).

ho_203(A,B,P,Q)  :- empty(A),call(P,B).

ho_203(A,B,P,Q)  :- head(A,C),tail(A,D),ho_203(D,E,P,Q),call(Q,E,C,B).

ho_608(A,B,P,Q,R)  :- empty(A),call(P,B).

ho_608(A,B,P,Q,R)  :- call(Q,A,C),tail(A,D),ho_608(D,E,P,Q,R),call(R,C,E,B).

ho_35(A,P)  :- empty(A).

ho_35(A,P)  :- head(A,B),tail(A,C),call(P,B),ho_35(C,P).

ho_152(A,B,C,P,Q)  :- call(P,A),eq_list(B,C).

ho_152(A,B,C,P,Q)  :- decrement(A,D),ho_152(D,B,E,P,Q),call(Q,E,C).
...
ho_493(A,B,P,Q)  :- cons3(A,C,D),call(P,C),ho_493(D,B,P,Q).

ho_493(A,B,P,Q)  :- cons3(A,C,D),call(Q,C),ho_493(D,E,P,Q),cons(C,E,B).
