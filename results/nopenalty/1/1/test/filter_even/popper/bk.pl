...
ho_124(A,B,P)  :- head(A,C),tail(A,D),head(B,E),tail(B,F),call(P,C,E),ho_124(D,F,P).

ho_280(A,B,P,Q,R)  :- empty(A),zero_int(B).

ho_280(A,B,P,Q,R)  :- head(A,C),tail(A,D),call(P,C),ho_280(D,B,P,Q,R).

ho_280(A,B,P,Q,R)  :- head(A,C),tail(A,D),call(Q,C),ho_280(D,E,P,Q,R),call(R,E,B).

ho_149(A,B,C,P,Q)  :- call(P,A),eq_list(B,C).

ho_149(A,B,C,P,Q)  :- decrement(A,D),ho_149(D,B,E,P,Q),call(Q,E,C).

ho_46(A,P,Q,R)  :- call(P,A).

ho_46(A,P,Q,R)  :- call(Q,A,B),tail(A,C),call(R,B),ho_46(C,P,Q,R).
