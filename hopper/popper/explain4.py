...
class Explainer:
    ...
    def explain_totally_incomplete(self, rewriter, prog, directions):
        return self.explain_totally_incomplete_aux(rewriter, prog, directions, 0, set(), set())

    def explain_totally_incomplete_aux(self, rewriter, prog, directions, depth, sat=set(), unsat=set()):
        ...
        for subprog in find_subprogs(prog, has_recursion):
            ...
            test_prog = self.build_test_prog(test_subprog, directions)
            ...
            unsat.add(raw_prog)
            xs = list(self.explain_totally_incomplete_aux(rewriter, subprog, directions, depth+1, sat, unsat))
            if len(xs):
                yield from xs
            else:
...
def find_subrules(rule, force_head, recursive):
    for rule in find_subrules_aux(rule, force_head, recursive):
        ...
        # head output arg in a base case must appear in the body
...
def order_body(body):
    ...
    while body_literals:
        ...
        if selected_literal == None:
            message = f'{selected_literal} could not be grounded'
