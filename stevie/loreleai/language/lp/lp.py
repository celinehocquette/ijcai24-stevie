...
class ClausalTheory(Theory):
    ...
    def __init__(self, formulas: Sequence[Clause] = None, read_from_file: str = None, preserve_definitions=False):
        assert formulas is not None or read_from_file is not None

        if read_from_file and preserve_definitions:
            formulas = []
            formulas_per_head = {}
            ...
            self._recursive_predicates = [x.get_head()[0].get_predicate() for x in formulas if isinstance(x, Recursion)]
...
def _convert_to_atom(string: str):
    ...
    if is_negated:
        return Not(c_literal(pred, args))
    else:
