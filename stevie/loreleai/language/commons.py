...
class SecondOrderVariable(Variable):

    def __init__(self, name: str):
        super(SecondOrderVariable, self).__init__(name, global_context.type('higher_order'))
...
@dataclass
class Not(Formula):
    def __init__(self, formula: 'Literal'):
    ...
    def get_formula(self) -> 'Literal':
        return self.formula

    def get_predicate(self) -> Predicate:
        return self.formula.get_predicate()

    def get_predicates(self) -> Set[Predicate]:
        return self.formula.get_predicates()

    def is_higher_order(self):
        return self.formula.is_higher_order()
    ...
    def __repr__(self):
        return f"not({self.get_formula()})"


@dataclass
class Literal(Formula):
    def __init__(self, predicate: Union[Predicate, SecondOrderVariable], arguments: List[Union[Term,Predicate]]):
    ...
    def is_higher_order(self):
        return isinstance(self.predicate, SecondOrderVariable)
    ...
    def __repr__(self):
        return "{}({})".format(
            self.predicate.get_name(), ",".join([x.get_name() if isinstance(x, Predicate) else str(x) for x in self.arguments])
        )
...
class Clause(Formula):
    ...
    def alphabeticise(self, from_pred_name=False):
        """
            Transforms the clause so that the variables appear in the alphabetic order
        """
        new_var_map = {}
        new_pred_map = {}
        new_pred_map[self._head.predicate.name] = 0
        for var in self._head.get_variables():
            if var not in new_var_map:
                if isinstance(var, SecondOrderVariable):
                    pass
                    #new_var_map[var] = SecondOrderVariable(f"V{len(new_var_map)}")
                elif from_pred_name:
                    new_var_map[var] = Variable(f"V{self._head.predicate.name}{new_pred_map[self._head.predicate.name]}", var.get_type())
                    new_pred_map[self._head.predicate.name] += 1
                else:
                    new_var_map[var] = Variable(f"{chr(ord('A') + len(new_var_map))}", var.get_type())

        for lit in self._body:
            is_negated = True if isinstance(lit, Not) else False
            pred_name = lit.formula.predicate.name if is_negated else lit.predicate.name
            if pred_name not in new_pred_map:
                new_pred_map[pred_name] = 0
            for var in lit.get_variables():
                if var not in new_var_map:
                    if isinstance(var, SecondOrderVariable):
                        pass
                        #new_var_map[var] = Variable(f"V{len(new_var_map)}", var.get_type())
                    elif from_pred_name:
                        new_var_map[var] = Variable(f"V{pred_name}{new_pred_map[pred_name]}", var.get_type())
                        new_pred_map[pred_name] += 1
                    else:
                        new_var_map[var] = Variable(f"{chr(ord('A') + len(new_var_map))}", var.get_type())
        self._head = self._head.substitute(new_var_map)
        self._body = [x.substitute(new_var_map) for x in self._body]
    ...
    def is_higher_order(self):
        return any([x.is_higher_order() for x in self._body])
    ...
    def is_recursive(self) -> bool:
        ...
        """
        return any([x.get_formula().get_predicate() == self._head.get_predicate() if isinstance(x, Not) else x.get_predicate() == self._head.get_predicate() for x in self._body])
    ...
    def _get_atom_order(self):
        ...
        while len(all_atoms) > 0:
            tmp = [t for t in all_atoms]
            tmp = [t.get_variables() for t in all_atoms]
            matching_atms = [
                x
                for x in all_atoms
            ...
            for x in all_atoms:
                if isinstance(x, Not):
                    found = False
                    for y in matching_atms:
                        if isinstance(y, Not) and x.formula == y.formula:
                            found = True
                    if not found:
                        new_atoms.append(x)
                else:
                    if x not in matching_atms:
                        new_atoms.append(x)
            all_atoms = new_atoms
            # all_atoms = [x for x in all_atoms if x not in matching_atms]
            all_vars_from_matching_atoms = reduce(
                (lambda x, y: x + y),
                [x.get_variables() for x in matching_atms],
                []
            )
            focus_vars = [v for v in all_vars_from_matching_atoms if v not in processed_vars]

            if len(focus_vars) == 0:
                focus_vars = [x for x in head_vars if x not in processed_vars]
                focus_vars = focus_vars[:1]
    ...
    def __repr__(self):
        if self._repr_cache is None:
            ...
            #     matching_atms = sorted(matching_atms, key=lambda x: ifthenelse_min([x.get_variables().index(y) if y in x.get_variables() else 5 for y in focus_vars]))
            #     processed_vars = processed_vars.union(focus_vars)
            #     atom_order += matching_atms
            #     all_atoms = [x for x in all_atoms if x not in matching_atms]
...
class ClausalConstruct(ABC):
    ...
    def __hash__(self):
        return hash([hash(x) for x in self._clauses])
...
class Disjunction(ClausalConstruct):
    ...
    def __hash__(self):
        return hash(tuple([hash(x) for x in self._clauses]))
    ...
    def alphabeticise(self, from_pred_name=False):
        ...
        for c in self._clauses:
            c.alphabeticise(from_pred_name=from_pred_name)
            new_clauses.append(Clause(c._head, c._body))
...
class Recursion(ClausalConstruct):
    ...
    def __init__(self, clauses: Sequence[Clause]):
        ...
        non_recursive = [x for x in clauses if not x.is_recursive()]
        recursive = [x for x in clauses if x.is_recursive()]
        super(Recursion, self).__init__(non_recursive + recursive)
...
class Context:
    ...
    def higher_order_variable(self, name):
        if name not in self._higher_order_variables:
            self._higher_order_variables[name] = SecondOrderVariable(name)
        return self._higher_order_variables[name]
...
def c_hvar(name, ctx: Context = None) -> SecondOrderVariable:
    ctx = _get_proper_context(ctx)
    return ctx.higher_order_variable(name)
...
def alphabeticiseClausalConstruct(clauses, from_pred_name=False):
    if isinstance(clauses, Clause):
        clauses.alphabeticise(from_pred_name=from_pred_name)
        return Clause(clauses._head, clauses._body)
    elif isinstance(clauses, Recursion):
        clauses.alphabeticise(from_pred_name=from_pred_name)
        return Recursion(clauses._clauses)
    elif isinstance(clauses, Disjunction):
        clauses.alphabeticise(from_pred_name=from_pred_name)
        return Disjunction(clauses._clauses)